...
var SupabaseManager = class {
  ...
  async upsertFile(path, content, checksum, deviceId) {
    const { error } = await this.getClient().from("files").upsert({
      path,
      content,
      checksum,
      device_id: deviceId,
  ...
  async deleteFile(path) {
    const client = this.getClient();
    const { error: tError } = await client.from("tombstones").upsert({
      path,
      deleted_at: (/* @__PURE__ */ new Date()).toISOString()
    });
    if (tError)
      throw tError;
    const { error: fError } = await client.from("files").delete().eq("path", path);
    if (fError)
      throw fError;
  }
  async getRemoteIndex() {
    const { data, error } = await this.getClient().from("files").select("path, updated_at, checksum");
    if (error)
      throw error;
    return data || [];
  }
  async getTombstones() {
    const { data, error } = await this.getClient().from("tombstones").select("path, deleted_at");
    if (error)
      throw error;
    return data || [];
  }
  async getIncrementalSync(since) {
    const { data, error } = await this.getClient().from("files").select("path, updated_at, checksum, content").gt("updated_at", since);
    if (error)
      throw error;
    return data || [];
  }
  async uploadBinary(path, buffer) {
    const client = this.getClient();
    const fileName = `${Date.now()}-${path.replace(/\//g, "_")}`;
    const { data, error } = await client.storage.from("vault-attachments").upload(fileName, buffer, {
      contentType: "application/octet-stream",
      upsert: true
    });
    if (error)
      throw error;
    const { data: { publicUrl } } = client.storage.from("vault-attachments").getPublicUrl(fileName);
    return publicUrl;
  }
};
var supabaseManager = new SupabaseManager();
...
var isDesktop = import_obsidian3.Platform.isDesktop;
var isMobile = import_obsidian3.Platform.isMobile;
var isIosApp = import_obsidian3.Platform.isIosApp;
var isMacApp = import_obsidian3.Platform.isMacApp;
var PlatformHelper = class {
  static getRealtimeConfig() {
    if (isMobile) {
      return { timeout: 3e4, heartbeatIntervalMs: 2e4 };
    }
    return { timeout: 15e3, heartbeatIntervalMs: 1e4 };
  }
  static shouldDeferHeavySync() {
    return isMobile;
  }
  static getDebounceMs(userSetting) {
    if (isMobile) {
      return Math.max(userSetting, 4e3);
    }
    return userSetting;
  }
  static canUseBackgroundSync() {
    return false;
  }
  static getMaxBatchSize() {
...
function md5Checksum(content) {
  return (0, import_blueimp_md5.default)(content);
}
...
var OfflineQueue = class {
  constructor(plugin) {
    ...
    var _a;
    this.plugin = plugin;
    this.queue = ((_a = this.plugin.loadData()) == null ? void 0 : _a.offlineQueue) || [];
  }
  async add(item) {
...
var SyncEngine = class {
  constructor(plugin) {
    ...
    this.pendingRemoteWrites = /* @__PURE__ */ new Set();
    this.plugin = plugin;
  ...
  updateStatus(status, detail) {
    ...
    if (detail) {
      this.statusBarItem.onClick(() => new import_obsidian4.Notice(detail));
    }
  }
  async start() {
    ...
    try {
      ...
      this.updateStatus("Sync error", e.message);
    }
  }
  async stop() {
  ...
  async initialReconciliation() {
    ...
    const excludePatterns = this.plugin.settings.excludePatterns.split(",").map((p) => (0, import_obsidian4.normalizePath)(p.trim()));
  ...
  async setupRealtimeListener() {
    ...
    channel.on("postgres_changes", { event: "*", schema: "public", table: "files" }, (payload) => {
  ...
  async handleLocalChange(type, file, oldPath) {
    var _a;
    if (!(file instanceof import_obsidian4.TFile))
      return;
    const path = file.path;
    if (this.pendingRemoteWrites.has(path))
      return;
    if (this.isExcluded(path))
      return;
    const debounce = PlatformHelper.getDebounceMs(this.plugin.settings.debounceMs);
    if (type === "modify") {
      if ((_a = this.debounceTimers) == null ? void 0 : _a[path])
        clearTimeout(this.debounceTimers[path]);
      this.debounceTimers = this.debounceTimers || {};
      this.debounceTimers[path] = setTimeout(() => this.processLocalChange(type, file), debounce);
      return;
    }
    await this.processLocalChange(type, file, oldPath);
  }
  async processLocalChange(type, file, oldPath) {
    const path = file.path;
    try {
      if (type === "delete") {
  ...
  async pullRemoteToLocal(remote) {
  ...
  async handleConflict(file, remote) {
  ...
  async getChecksum(file, overrideContent) {
    const content = overrideContent || await this.plugin.app.vault.read(file.path);
    return md5Checksum(content);
  }
  isExcluded(path) {
    const patterns = this.plugin.settings.excludePatterns.split(",").map((p) => p.trim());
...
// src/main.ts
var SupaSyncPlugin = class extends import_obsidian6.Plugin {
  async onload() {
